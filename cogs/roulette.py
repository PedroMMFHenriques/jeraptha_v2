...
class Roulette(commands.Cog):
    ...
    async def start(self, ctx: discord.ApplicationContext, betting_time: int):
        ...
        for winner in winners_list:
            if(winner["winnings"] > 0):
                ...
                winnings_embed += "<@" + str(winner["id"]) + "> won **" + str(math.floor(winner["winnings"])) + "** <:beets:1245402688565411841>!\n"
    ...
    async def bet(self, ctx: discord.ApplicationContext, amount: int, option: str, sub_option: str, list_numbers: str):
        ...
        await ctx.respond("[Roulette] <@" + str(ctx.author.id) + "> bet on **" + print_msg + "** with **" + str(amount) + "** <:beets:1245402688565411841>!")
